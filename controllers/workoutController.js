...
export async function updateTemplate(req, res) {
  try {
    ...
    if (!user_id || !template_name || !exercises || !notes || !date) {
    ...
    await knex("workout_templates").where({ id }).update({
      user_id,
      template_name,
      exercises,
      notes,
    ...
    console.log(`Error updating template with id: ${req.params.id}.${err}`);
    res.status(500).send(`Error updating template with id: ${req.params.id}`);
  }
}
